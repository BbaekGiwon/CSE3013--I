	...
	while (bfs_root.size() > 0) {
		...
		if (loc_y == maze_input.size() - 2 && loc_x == maze_input[0].size() - 2) {
			...
			for (int i = 0; i < bfs_root.size(); i++) {
				cout << bfs_root[i].first << ", " << bfs_root[i].second << "\n";
			}
			*/
			for (int i = 0; i < maze_input.size(); i++) {
				delete[] visited[i];
...
void ofApp::bfsdraw() {
	...
	//first draw every trial with red
	///*
	for (int i = 1; i < trial_bfs_root.size(); i++) {
	...
	//*/
	ofSetColor(0, 256, 256);
	//then draw answer bfs_root over every trail
	int idx_end_root = trial_bfs_root.size() - 1;
	auto find_end_root = trial_bfs_root[trial_bfs_root.size() - 1];
	while (get<0>(trial_bfs_root[idx_end_root]) != maze_input.size() - 2 && get<1>(trial_bfs_root[idx_end_root]) != maze_input[0].size() - 2) {
		//cout << idx_end_root << ", " << get<0>(trial_bfs_root[idx_end_root]) << ", " << get<1>(trial_bfs_root[idx_end_root]) << "\n";
		idx_end_root--;
	}

	while (get<2>(trial_bfs_root[idx_end_root]) != -1) {
		auto temp_bfs_root1 = trial_bfs_root[idx_end_root];
		auto temp_bfs_root2 = trial_bfs_root[get<2>(trial_bfs_root[idx_end_root])];
		ofDrawLine(get<1>(temp_bfs_root1) * 10, get<0>(temp_bfs_root1) * 10, get<1>(temp_bfs_root2) * 10, get<0>(temp_bfs_root2) * 10);
		idx_end_root = get<2>(trial_bfs_root[idx_end_root]);
	}
